...
def rand_rotate(img, label):
    ...
    img_rotate = np.asarray(sktransf.rotate(img, r, order=1, mode='symmetric',
                                            preserve_range=True), np.float64)
...
def rotate_image(image, angle, order=0):
    ...
    rot_mat_notranslate = np.asmatrix(rot_mat[0:2, 0:2])
    ...
    affine_mat = (np.asmatrix(trans_mat) * np.asmatrix(rot_mat))[0:2, :]
...
def RandomExchange(img1, img2, label1, label2):
    r = random.random()
    if r < 0.5:
        return img1, img2, label1, label2
    else:
